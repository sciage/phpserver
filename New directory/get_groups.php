...
$sql = "SELECT groups.group_id, groups.name as group_name, groups.group_image_url, goups.group_description, (Select count(id_user_name) as users_in_group from groups_user where group_id = groups.group_id) as users_in_group, (Select count(group_id) as posts_inside_group from posts where group_id = groups.group_id) as posts_inside_groups, groups.id_categories FROM groups, (select category from categories where categories.id_categories = groups.id_categories) as category_name, groups.created_by_id_user_name as created_by_id_user_name WHERE  id_categories $joined_condition in (SELECT id_categories FROM group_category_join where id_user_name = $user_id) $below18_condition $limit_clause";
