...
if(!empty($_GET['gender']) && !empty($_GET['id_user_name'])){
    $q=mysqli_query($con,"SELECT id_posts
				FROM posts
				WHERE id_user_name
				IN (
					SELECT id_user_name FROM candid_database.user_name where gender = 
					'".$_GET['gender']."') order by post_time desc;
				");
...
else if(!empty($_GET['user_date_of_birth']) && !empty($_GET['id_user_name'])){
    $q=mysqli_query($con,"SELECT id_posts
				FROM posts
				WHERE id_user_name
				IN (
					SELECT id_user_name FROM candid_database.user_name where user_date_of_birth = 
					'".$_GET['gender']."') order by post_time desc;
				");
    ...
    if(count($post_ids)){
        unset($_GET['id_user_name']);
        unset($_GET['user_date_of_birth']);
        $_GET['id_posts']=implode(",", $post_ids);
    }else{
        die('[]');
    }
} else if(!empty($_GET['user_date_of_birth']) && !empty($_GET['id_user_name']) && $_GET['gender'] ){
    $q=mysqli_query($con,"SELECT id_posts
				FROM posts
				WHERE id_user_name
				IN (
