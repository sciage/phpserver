<?php
	...
	$sql = mysqli_prepare($con, "DELETE FROM `group_category_join` WHERE id_categories = ? AND id_user_name=  ? LIMIT 1");
	...
	if ($sql) {
		foreach ($group_ids as $key => $group_id) {
			...
			if( is_numeric($group_id) ){
			    ...
			    if( mysqli_stmt_execute($sql) && mysqli_affected_rows($con) ){
			    	$successful_queries++;
			    }
