...
$deviceId = isset($_POST['deviceId']) ? mysqli_real_escape_string($con, $_POST['deviceId']) : $bytes;
    ...
    if (mysqli_num_rows($email_id_duplicate) > 0) {
        ...
        mysqli_query($con, "UPDATE `user_name` SET  `name`='$name',`email`='$email',`userid` ='$user_id',`user_registered`='$created', `user_token`='$bytes',`deviceId`='$deviceId',`socialNetwork`='$socialNetwork' WHERE `userid` ='$user_id' && `email` ='$email' LIMIT 1");
        ...
        mysqli_query($con, "UPDATE `user_name` SET  `name`='$name',`email`='$email',  `userid` ='$randomuserID', `user_registered`='$created', `user_token`='$bytes' ,`deviceId`='$deviceId',`socialNetwork`='$socialNetwork'  WHERE `deviceId` ='$deviceId' LIMIT 1");
